...
def main(args: argparse.Namespace, config):
    ...
    if args.sources is None:
        args.sources = list(image_list[args.dset].keys())
        args.sources.remove(args.target)

    print("Source: {} Target: {}".format(args.sources, args.target))

    train_source_dataset = ConcatDataset([
        ImageList(open(os.path.join(args.root, "{}_{}.txt".format(image_list[args.dset][source], 'train'))).readlines(), transform=train_transform)
        for source in args.sources])
    train_source_loader = DataLoader(train_source_dataset, batch_size=args.batch_size,
                                     shuffle=True, num_workers=args.workers, drop_last=True)
    train_target_dataset = ImageList(open(os.path.join(args.root, "{}_{}.txt".format(image_list[args.dset][args.target], 'train'))).readlines(), transform=train_transform)
    train_target_loader = DataLoader(train_target_dataset, batch_size=args.batch_size,
                                     shuffle=True, num_workers=args.workers, drop_last=True)
    val_dataset = ImageList(open(os.path.join(args.root, "{}_{}.txt".format(image_list[args.dset][args.target], 'train'))).readlines(), transform=val_tranform)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    if args.dset == 'DomainNet':
        test_dataset = ImageList(open(os.path.join(args.root, "{}_{}.txt".format(image_list[args.dset][args.target], 'test'))).readlines(), transform=val_tranform)
    ...
    for epoch in range(args.epochs):
        ...
        if args.dset == "DomainNet":
            if epoch >= args.start_test_epoch:
...
def train(train_source_iter: ContinuousDataloader, train_target_iter: ContinuousDataloader, model: ImageClassifier,
    ...
    for i in range(args.iters_per_epoch):
        ...
        (x_s, labels_s, idx_source), domain_id = next(train_source_iter)
        ...
        if i % args.print_freq == 0:
            print(
                "Epoch: [{:02d}][{}/{}]	total_loss:{:.3f}	aug_loss:{:.3f}	 pro_loss:{:.3f}  MI_loss:{:.3f}".format( \
                    epoch, i, args.iters_per_epoch, total_loss, aug_loss, pro_loss, MI_loss))
            config["out_file"].write(
                "Epoch: [{:02d}][{}/{}]	total_loss:{:.3f}	aug_loss:{:.3f}	 pro_loss:{:.3f}  MI_loss:{:.3f}\n".format( \
                    epoch, i, args.iters_per_epoch, total_loss, aug_loss, pro_loss, MI_loss))
            config["out_file"].flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dset', type=str, default='office-home', choices=['office-home', 'DomainNet'], help="The dataset used")
    parser.add_argument('--root', type=str, default='/data1/TL/data/office-home-65/', help="The source dataset root")
    parser.add_argument('--sources', nargs='+', default=None, help='source domain(s). Use all domains except the target domain if set None')
    parser.add_argument('--target', type=str, default='a', help="The target test dataset path list")
    parser.add_argument('--output_dir', type=str, default='log/home', help="output directory of logs")
    ...
    parser.add_argument('--start_test_epoch', type=int, default=0, help="start_test_epoch")
    ...
    config["out_file"] = open(osp.join(args.output_dir, get_current_time() + "_" + args.target + "_log.txt"), "w")


    config["out_file"].write("file name: train_TTSA_for_MSDA\n")
