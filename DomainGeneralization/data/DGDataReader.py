...
class DGDataset(Dataset):
    def __init__(self, datalists_folder, names, labels, transformer=None):
        self.datalists_folder = datalists_folder
        self.names = names
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.datalists_folder, img_name)).convert('RGB')
...
class FourierDGDataset(Dataset):
    def __init__(self, datalists_folder, names, labels, transformer=None, from_domain=None, alpha=1.0):
        self.datalists_folder = datalists_folder
        self.names = names
        self.labels = labels
        self.transformer = transformer
    ...
    def sample_image(self, domain):
        ...
        img_sampled = Image.open(os.path.join(self.datalists_folder, imgn_ame_sampled)).convert('RGB')
        label_sampled = self.labels[domain_idx][img_idx]
        return self.transformer(img_sampled), label_sampled, domain_idx


def get_dataset(datalists_folder, path, train=False, image_size=224, crop=False, jitter=0, config=None):
    ...
    return DGDataset(datalists_folder, names, labels, img_transform)


def get_fourier_dataset(datalists_folder, path, image_size=224, crop=False, jitter=0, from_domain='all', alpha=1.0, config=None):
    ...
    return FourierDGDataset(datalists_folder, names, labels, img_transform, from_domain, alpha)