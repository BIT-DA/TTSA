...
def get_datalists_folder(data_dir):
    datalists_folder = data_dir
    return datalists_folder


def get_train_dataloader(source_list=None, batch_size=64, image_size=224, crop=False, jitter=0, data_dir=None, config=None):
    assert isinstance(source_list, list)
    datasets = []
    for dname in source_list:
        datalists_folder = get_datalists_folder(data_dir)
        path = os.path.join(datalists_folder, '%s_train.txt' % dname)
        train_dataset = get_dataset(datalists_folder=datalists_folder,
                                    path=path,
                                    train=True,
...
def get_fourier_train_dataloader(
    ...
    for dname in source_list:
        ...
        path = os.path.join(datalists_folder, '%s_train.txt' % dname)
        paths.append(path)
    dataset = get_fourier_dataset(datalists_folder=datalists_folder,
                                  path=paths,
                                  image_size=image_size,
                                  crop=crop,
                                  jitter=jitter,
...
def get_val_dataloader(source_list=None, batch_size=64, image_size=224, data_dir=None, config=None):
    assert isinstance(source_list, list)
    datasets = []
    for dname in source_list:
        datalists_folder = get_datalists_folder(data_dir)
        path = os.path.join(datalists_folder, '%s_val.txt' % dname)
        val_dataset = get_dataset(datalists_folder=datalists_folder, path=path, train=False, image_size=image_size, config=None)
...
def get_test_loader(target=None, batch_size=64, image_size=224, data_dir=None, config=None):
    data_folder = get_datalists_folder(data_dir)
    path = os.path.join(data_folder, '%s_test.txt' % target)
    test_dataset = get_dataset(datalists_folder=data_folder, path=path, train=False, image_size=image_size, config=None)
